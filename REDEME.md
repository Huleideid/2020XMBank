...
| behavior_m.csv | **B5-B3**(B5-B3),  **B5/B3**(B5/B3）                         |
...
​		这里的后处理采用加类别权重的训练方式进行训练，权重系数直接采用[2]，据说是搜索得到，细节有待补充。当然也可以参考[4]，亲测有效。由于赛后重新整理了程序，做了一些新的特征，最新的提交结果未测，估计二者单独均在0.49左右，单LGM线下更高。如果两者的特征完全相同，两者融合可能并不能带来提升。
